package br.com.fiap.techchallenge.quickserveapi.controller;
...
import br.com.fiap.techchallenge.quickserveapi.utils.BuildOrderPaymentStatusDTO;
...
public class OrderApiTest {
    ...
    class BuscarPedido {
        ...
        void devePermitirBuscarPedido() throws Exception {
            ...
            when(orderController.findById(any(Long.class))).thenReturn(BuildOrderResponseDTO.buildOrderDTO());

            mockMvc.perform(get(urlOrder+"/{id}",id)).andExpect(status().isOk());

            verify(orderController, times(1)).findById(any(Long.class));
        }
        ...
        void devePermitirBuscarStatusPagamento() throws Exception {
            ...
            when(orderController.checkPaymentStatus(any(Long.class))).thenReturn(BuildOrderPaymentStatusDTO.buildOrderStatusDTO());

            mockMvc.perform(get(urlOrder+"/payment/{id}",id)).andExpect(status().isOk());

            verify(orderController, times(1)).checkPaymentStatus(any(Long.class));
        }
        ...
        void deveGerarExcecao_QuandoBuscarStatusPagamento_IdNaoExiste() throws Exception {
            ...
            // Simula a exceção sendo lançada no controlador
            when(orderController.checkPaymentStatus(any(Long.class))).thenThrow(NotFoundException.class);
    ...
    class ListarPedidos{
        ...
        @Test
        void deveRetornarErroGenericoQuandoOcorreExcecao() throws Exception {
            // Mock para simular uma exceção genérica
            when(orderController.findAll()).thenThrow(new RuntimeException("Unexpected error"));

            // Realiza a requisição GET e valida o status de erro genérico
            mockMvc.perform(get(urlOrder + "/"))
                    .andDo(print())
